...
- **Native menu bar** — TerraNova, File, Edit, View, Window and Help. Settings sits under the app menu at `Cmd+,` where macOS users expect it. The Edit menu is not cosmetic: macOS routes copy, paste and undo through menu items, so text fields now behave correctly in the desktop app.
...
- Crashes are written to `~/Library/Logs/TerraNova/crash.log`, the standard macOS location, at the moment they happen — so a force quit still leaves evidence, and the log survives the reload that used to erase it.
- A blank window now explains itself instead of sitting empty: the error and component stack are shown in-app, and a stalled startup, a lost WebGL context, or a splash overlay that never lifted are each named rather than looking identical.

### Settings

- Reachable from Home. It was previously only accessible from the editor or first-run onboarding, so with no project open there was no way in at all.
- The panel no longer resizes when you switch categories, and the sidebar fits every category name.
- **Launch opens to Home** rather than reopening the last pack. Reopening is opt-in under General.

### Worldgen accuracy (Hytale Update 6)

- WhiteNoise implemented bit-exact against the Update 6 generator.
- MultiMix's keyed inputs are wired positionally, matching how Hytale reads them.
- V2's Constant vector provider is read correctly.
- Noise parity fixtures rebaselined on the Update 6 jar.
...
- Sign-in handles real browser behaviour on the OAuth callback.
