# Walkthroughs

Step-by-step practical tutorials for common TerraNova tasks. Each walkthrough takes you through a real workflow from start to finish.

## Available Walkthroughs

- **[Create a World](./create-a-world.md)** — build your first world from a blank canvas: add terrain nodes, assign materials, and save.
- **[Shaping Terrain and Carving Caves](./terrain-and-caves.md)** — step-by-step from flat plains → rolling hills → mountains → overhangs → caves, with tuning guides for each.
- **[Setting Up a Multi-Biome World](./multi-biome-world.md)** — create three biomes, assign noise ranges, configure transitions, and tune boundaries.
- **[Periodic Density Stripes ⚗](./periodic-density-stripes.md)** — use Modulo + CurveFunction to tile a density pattern at a fixed interval; stripes, sediment layers, and banded caves.

> More walkthroughs coming — props and prefab placement, materials and layering, and performance optimisation.
