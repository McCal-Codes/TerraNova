...
- **[Terrain Math Explained](./terrain-math-explained.md)** — the actual math behind every node type: what each parameter does, how combining nodes produces specific terrain shapes, and a quick-reference tuning table.
- **[Curves Explained](./curves-explained.md)** — what curve assets are, every curve type (DistanceExponential, DistanceS, Manual, Clamp, Floor, Ceiling, Inverter, and their smooth variants), when to use each, and a decision guide for picking the right curve for island edges, beaches, and shape falloffs.
- **[Materials Guide](./materials-guide.md)** — how material providers work: SpaceAndDepth layer types, ConstantThickness, NoiseThickness, RangeThickness, cave ceiling/floor detection with DownwardSpace/UpwardSpace, and worked examples for plains, desert, and cave biomes.
- **[Props & Placement](./props-and-placement.md)** — the position provider → scanner → prop pipeline, prop types (Prefab, Cluster, Weighted, PondFiller), position providers (Occurrence, Jitter2d, SimpleHorizontal), scanners (ColumnLinear, ColumnRandom, Area), and worked examples for trees, boulders, cave mushrooms, and ponds.
- **[Terrain Types and Node Recipes](./terrain-types.md)** — organized by terrain outcome: plains, mountains, mesas, floating islands (SDF), skylands altitude bands, caves, warped terrain, dunes, archipelagos, and complex layered worlds.
