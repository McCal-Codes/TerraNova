...
OUT_FILE="$REPO_ROOT/src/utils/density/__tests__/fixtures/fbmParity.json"
...
echo "  $JAR"
javac -nowarn -cp "$JAR" -d "$BUILD_DIR" "$HERE/Parity.java"

mkdir -p "$(dirname "$OUT_FILE")"
echo "Sampling the real SimplexNoiseField..."
java -cp "$JAR:$BUILD_DIR" com.hypixel.hytale.builtin.hytalegenerator.noise.Parity > "$OUT_FILE"

echo "Wrote $OUT_FILE ($(wc -c < "$OUT_FILE" | tr -d ' ') bytes)"
echo "Now run: pnpm vitest run src/utils/density/__tests__/fbmParity.test.ts"
