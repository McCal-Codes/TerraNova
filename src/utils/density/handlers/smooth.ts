...
const handleSmoothClamp: NodeHandler = (ctx, fields, inputs, x, y, z) => {
  ...
  // V2: WallA = upper, WallB = lower. Legacy fallback: Max/Min.
  const upper = Number(fields.WallA ?? fields.Max ?? 1);
  const lower = Number(fields.WallB ?? fields.Min ?? -1);
  const k = Number(fields.Range ?? fields.Smoothness ?? 0.01);
  return smoothMax(smoothMin(v, upper, k), lower, k);
};
...
const handleSmoothFloor: NodeHandler = (ctx, fields, inputs, x, y, z) => {
  ...
  const k = Number(fields.WallB ?? fields.Smoothness ?? fields.SmoothRange ?? 1.0);
...
const handleSmoothMin: NodeHandler = (ctx, fields, inputs, x, y, z) => {
  const k = Number(fields.SmoothRange ?? fields.Range ?? fields.Smoothness ?? 1.0);
...
const handleFloor: NodeHandler = (ctx, fields, inputs, x, y, z) => {
  const v = ctx.getInput(inputs, "Input", x, y, z);
  const floor = Number(fields.Limit ?? fields.Floor ?? 0);
  return Math.max(v, floor);
};

const handleCeiling: NodeHandler = (ctx, fields, inputs, x, y, z) => {
  const v = ctx.getInput(inputs, "Input", x, y, z);
  const ceiling = Number(fields.Limit ?? fields.Ceiling ?? 1);
