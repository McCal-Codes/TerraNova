...
const handleInverter: NodeHandler = (ctx, _fields, inputs, x, y, z) => {
...
const handleSqrt: NodeHandler = (ctx, _fields, inputs, x, y, z) => {
...
const handleMultiplier: NodeHandler = (ctx, _fields, inputs, x, y, z) => {
  let prod = 1;
  for (let i = 0; inputs.has(`Inputs[${i}]`); i++) {
...
export function buildArithmeticHandlers(): Map<string, NodeHandler> {
  return new Map<string, NodeHandler>([
    ["Inverter", handleInverter],
    ["Negate", handleInverter],
    ["Abs", handleAbs],
    ["Sqrt", handleSqrt],
    ["SquareRoot", handleSqrt],
