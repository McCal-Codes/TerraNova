...
| `YSampled` | Wraps any subgraph and samples it at coarse Y intervals, then interpolates. Gives ~4× speedup with minimal visual change. |
...
| `Normalizer` | Remaps a value range, e.g. [–2, 2] → [–1, 1]. Use after `Sum` to keep density in expected range. |
| `Inverter` | Multiplies by –1. Flips solid and empty — used to turn noise into a cave mask. |
| `Scale` | Multiplies a value by a constant. Amplifies or reduces the contribution of a node. |
| `Clamp` | Limits output to a min/max range. Prevents runaway values from other operations. |
