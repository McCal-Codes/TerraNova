...
"Terrain": {
  ...
  "Inputs": [
    ...
    { "Type": "SimplexNoise2D", "Frequency": 0.01, "Amplitude": 8 }
  ]
}
```
...
"Terrain": {
  ...
  "Inputs": [
    ...
    { "Type": "CurveMapper", "Input": { "Type": "SimplexNoise2D", "Frequency": 0.005 } }
  ]
}
```

Two biomes can share a base structure but differ in parameters -- e.g. same noise type, different `Height` and `Frequency`.
...
| Type | Behavior |
|------|-----------|
| `EnvironmentConstant` | Fixed environment settings (fog color, density, sun angle) |
| `EnvironmentDensityDelimited` | Switches environment below/above a density threshold (e.g. underwater) |

**TintProvider** -- color tinting on blocks and foliage:

| Type | Behavior |
|------|-----------|
| `TintConstant` | One color applied everywhere (`Color` field, hex string) |
| `TintDensityDelimited` | Different tints above/below a density boundary |
